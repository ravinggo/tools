...
func (p *stringer) Generate1(file *generator.FileDescriptor) {
	proto3 := gogoproto.IsProto3(file.FileDescriptorProto)
	p.PluginImports = generator.NewPluginImports(p.Generator)
	p.atleastOne = false
	...
	for _, message := range file.Messages() {
		...
		for _, field := range message.Field {
			...
			if (field.IsMessage() && !gogoproto.IsCustomType(field)) || p.IsGroup(field) {
				...
				if nullable {
					...
					p.P(
						repeatedName, " += ", stringsPkg.Use(), `.Replace(`, stringsPkg.Use(), `.Replace(`, stringfunc, `, "`, typeName, `","`, msgname, `"`,
						", 1),`&`,``,1)", ` + ","`,
					)
				} else {
		...
		for _, field := range message.Field {
			...
			if oneof {
				...
				if nullable && !repeated {
					...
					p.P(
						"`", fieldname, ":`", ` + `, stringsPkg.Use(), `.Replace(`, stringsPkg.Use(), `.Replace(`, stringfunc, `, "`, typeName, `","`, msgname,
						`"`, ", 1),`&`,``,1) + `,", "`,",
					)
				}
			} else {
		...
		for _, field := range message.Field {
			...
			if field.IsMessage() || p.IsGroup(field) {
				...
				typeName := msgnames[len(msgnames)-1]
				p.P(
					"`", fieldname, ":`", ` + `, stringsPkg.Use(), `.Replace(`, fmtPkg.Use(), `.Sprintf("%v", this.`, fieldname, `), "`, typeName, `","`,
					msgname, `"`, ", 1) + `,", "`,",
				)
			} else {
				p.P("`", fieldname, ":`", ` + `, fmtPkg.Use(), `.Sprintf("%v", this.`, fieldname, ") + `,", "`,")
