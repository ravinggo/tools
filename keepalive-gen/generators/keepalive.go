...
func NewGenKeepAlive(outputFilename, targetPackage string, usePool bool) generator.Generator {
	...
	dc.imports = generator.NewImportTrackerForPackage(
		targetPackage,
		&types.Type{Name: types.Name{Package: "github.com/ravinggo/tools/keepalive-gen/keepalive"}},
...
func getIDType(t *types.Type) *types.Type {
	...
	for _, v := range t.Members {
		if strings.Contains(v.Tags, "key_id") {
			if v.Type.Kind != types.Builtin {
				klog.Fatalf("getIDType(%s) key_id type is not Builtin", t.Name.String())
			}
			return v.Type
		}
	}
